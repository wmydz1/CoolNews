...
import com.logoocc.dao.LinkDao;
...
import com.logoocc.util.DateUtil;
...
public class NewsServlet extends HttpServlet {
	...
	private String genUpAndDownPageCode(List<News> upAndDownPage) {
		...
		if (upNews.getNewsId() == -1) {
			...
			pageCode.append("<p>上一篇:<a href='news?action=show&newsId=" + upNews.getNewsId() + "'>" + upNews.getTitle()
					+ "</a></p>");
		}
		...
		if (downNews.getNewsId() == -1) {
			...
			pageCode.append("<p>下一篇:<a href='news?action=show&newsId=" + downNews.getNewsId() + "'>"
	...
	private void newsList(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			request.setAttribute("navCode",
					NavUtil.genNewsListNavigation(newsTypeDao.geNewsTypeById(con, typeId).getTypeName(), typeId));
